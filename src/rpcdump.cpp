...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <AlphaBoomprivkey> [label] [rescan=true]\n"
    ...
    if (fWalletUnlockMintOnly) // AlphaBoom: no importprivkey in mint-only mode
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AlphaBoom address or script");
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <AlphaBoomaddress>\n"
            "Reveals the private key corresponding to <AlphaBoomaddress>.");
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <AlphaBoomaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <AlphaBoomaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
