...
public:
    ...
    {
        // AlphaBoom: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // AlphaBoom: get transaction coin age
...
public:
    ...
    // AlphaBoom: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // AlphaBoom: entropy bit for stake modifier if chosen by modifier
    unsigned int GetStakeEntropyBit(unsigned int nHeight) const
    {
        // Protocol switch to support p2pool at AlphaBoom block #9689
        ...
        // Before AlphaBoom block #9689 - get from pregenerated table
    ...
    // AlphaBoom: two types of block: proof-of-work or proof-of-stake
    ...
    // AlphaBoom: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // AlphaBoom: calculate total coin age spent in block
...
public:
    ...
    uint32_t nFile;
    uint32_t nBlockPos;
    uint256 nChainTrust; // AlphaBoom: trust score of block chain
    int32_t nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    uint32_t nFlags;  // AlphaBoom: block index flags
    enum  
    {
