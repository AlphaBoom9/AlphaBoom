...
void Shutdown(void* parg)
{
    ...
    RenameThread("AlphaBoom-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("AlphaBoom version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  AlphaBoomd [options]                     " + "\n" +
                  "  AlphaBoomd [options] <command> [params]  " + _("Send command to -server or AlphaBoomd") + "\n" +
                  "  AlphaBoomd [options] help                " + _("List commands") + "\n" +
                  "  AlphaBoomd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "AlphaBoom:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("AlphaBoom"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("AlphaBoom"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: AlphaBoom.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: AlphaBoomd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  AlphaBoom is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "AlphaBoom server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("AlphaBoom"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (mapArgs.count("-reservebalance")) // AlphaBoom: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // AlphaBoom: checkpoint master priv key
    {
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of AlphaBoom") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart AlphaBoom to complete") << "\n";
